...
2. 改釘 `securego/gosec@v2`；其餘 actions 逐一檢查均為 major tag（checkout@v6、setup-go@v6、upload-artifact@v4、golangci-lint-action@v9、codeql-action/*@v4、upload-sarif@v4、govulncheck-action@v1、osv-scanner-action@v2、gitleaks-action@v2）。
...
- `.github/workflows/security.yml` — Security 四 job：govulncheck@v1；osv-scanner@v2（lockfile 模式，SARIF 上傳）；gosec@v2（`-no-fail -fmt sarif`，SARIF 上傳）；CodeQL（init→autobuild→analyze，config=`.github/codeql/codeql-config.yml`）
