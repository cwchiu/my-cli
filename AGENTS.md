...
5. **提交 PR**：PR description 必須包含 GitHub 自動關閉關鍵字，例如 `Closes #123` 或 `Fixes #123`，其中 `123` 必須是本次需求 Issue 編號。PR title、body 與 spec 要能對應同一個 Issue；未關聯 Issue 的 PR 不得合併。
   ...
   ```
6. **審核與合併**：等待 CI 全部通過並取得必要審核後，由 GitHub PR 合併到 `main`；不得以本地直接 merge 取代 PR 審核流程。**禁止 AI agent、automation、bot 或其他自動化流程自行批准、代替使用者批准或合併 PR；必須由人類 reviewer 完成審核，並由使用者明確授權合併。** 未取得明確授權時，只能回報 PR 狀態，不得執行 `gh pr merge`、GitHub merge API 或任何等效合併操作。確認 PR 狀態為 `MERGED`，且來源 Issue 已自動關閉。
...
> 使用者要求：**工作要可稽核、可復線**——每個工作項在 `specs/` 目錄留下一份紀錄檔，一個檔案對應一次工作內容。

1. **檔案命名**：`specs/<YYYYMMDD>-<topic>.md`（如 `specs/20260906-add-taskfile.md`），與 worktree 的 `<topic>` 一致。
2. **建立時機**：worktree 建立後即建立檔案骨架；工作過程中持續補充，合併前定稿。
3. **必載章節**（缺一不可）：
   - **Why**：為什麼做這件事（需求來源、動機）。
   - **What**：要做什麼（範圍、目標、非目標）。
   - **How**：怎麼做（步驟、技術選型理由）。
   - **遭遇的困難**：中間遇到什麼問題（錯誤訊息、誤判、環境限制）。
   - **如何解決**：每個困難對應的根因與解法。
